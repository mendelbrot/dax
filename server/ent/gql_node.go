...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case entry.Table:
		...
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, entryImplementors...); err != nil {
				return nil, err
			}
		}
	...
	case vault.Table:
		query := c.Vault.Query().
			Where(vault.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, vaultImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case entry.Table:
		query := c.Entry.Query().
			Where(entry.IDIn(ids...))
		query, err := query.CollectFields(ctx, entryImplementors...)
		if err != nil {
			return nil, err
		}
	...
	case vault.Table:
		query := c.Vault.Query().
			Where(vault.IDIn(ids...))
		query, err := query.CollectFields(ctx, vaultImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
