...
func (_q *UserQuery) QueryVaults() *VaultQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withVaults: _q.withVaults.Clone(),
...
func (_q *UserQuery) WithVaults(opts ...func(*VaultQuery)) *UserQuery {
	query := (&VaultClient{config: _q.config}).Query()
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withVaults; query != nil {
...
func (_q *UserQuery) loadVaults(ctx context.Context, query *VaultQuery, nodes []*User, init func(*User), assign func(*User, *Vault)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.VaultsTable)
		s.Join(joinT).On(s.C(vault.FieldID), joinT.C(user.VaultsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.VaultsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.VaultsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Vault](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "vaults" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
